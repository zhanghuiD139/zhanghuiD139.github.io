/* offline shell · v14（清掉 v13 及之前所有缓存；本次：仓库匹配改用 (项目号, 产品) 联合精确匹配——一个项目号项下多产品不同仓库）
   策略：页面(index.html)与 data.json 走「网络优先 / 直连网络」，保证每次部署都能拿到最新代码与数据；
        静态资源(图标/manifest/vendor OCR 引擎)走「缓存优先」，省流量。离线时回退缓存。
   文件名从 sw.v13.js 改为 sw.v14.js：旧 SW 缓存随之失效，强制全量重注册。 */
const C='gzt-v14';
...
self.addEventListener('activate',e=>{
  // 删除所有旧版本缓存（含 gzt-v1~v13 任意名），保证不残留冻结的旧 index.html / data.json
