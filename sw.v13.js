/* offline shell · v13（清掉 v12 及之前所有缓存；本次：出入库录入表单增强——项目编号 091/91/9 自动归一 + 产品/仓库自动联动 + 保存即自动复制 JSON + 顶部"待同步"卡片改文案）
   策略：页面(index.html)与 data.json 走「网络优先 / 直连网络」，保证每次部署都能拿到最新代码与数据；
        静态资源(图标/manifest/vendor OCR 引擎)走「缓存优先」，省流量。离线时回退缓存。
   文件名从 sw.v12.js 改为 sw.v13.js：旧 SW 缓存随之失效，强制全量重注册。 */
const C='gzt-v13';
...
self.addEventListener('activate',e=>{
  // 删除所有旧版本缓存（含 gzt-v1~v12 任意名），保证不残留冻结的旧 index.html / data.json
