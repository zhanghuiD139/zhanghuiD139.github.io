/* offline shell - v62（详情查询合同数量读取 sheet2 J 字段 + 产品规格空白字符规范化匹配）
   渐进说明：
     ...
     - v61: 详情查询按产品分组合同数量读 sheet2 J「合同数量」（一行=一个产品的合同总数），不再用 inQty+stock 推算（×2 错位）
     - v62: 详情查询产品匹配做空白字符规范化（去 \\n / 多空格 / 全角空格），sheet2 原值 vs sheet3 Lookup 规范化值不再因空白不一致匹配失败 */
const C='gzt-v62';
