/* offline shell · v5 (强制清掉 v4 缓存 + 刷新登录拦截逻辑)
   策略：页面(index.html)与 data.json 走「网络优先 / 直连网络」，保证每次部署都能拿到最新代码与数据；
        静态资源(图标/manifest)走「缓存优先」，省流量。离线时回退缓存。
   文件名从 sw.js 改为 sw.v4.js：旧 SW 缓存（冻结 167.5万 的旧 data.json）随之失效，强制全量重注册。 */
const C='gzt-v5';
...
self.addEventListener('activate',e=>{
  // 删除所有旧版本缓存（含 gzt-v1/v2/v3 任意名），保证不残留冻结的旧 data.json
...
self.addEventListener('fetch',function(e){
  ...
  /* data.json：永远直连网络、不读不写缓存，保证返回线上最新值（根治旧 SW 缓存冻结 167.5万 问题） */
  if(isData(req)){
