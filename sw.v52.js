/* offline shell · v52（预通知卡片 kv 超宽自动换行，不再被省略号截断）（合同详情币种纯净：USD只显示$，CNY只显示¥，去掉所有换算副标）（新增出入流程接入结果弹窗 + 网络瞬断重试 + 自检 250ms 间隔）（写入可靠性：客户端排障上报 + 自检探针）（v42 基础上只升版本号，强制用户拉到新 index.html）
