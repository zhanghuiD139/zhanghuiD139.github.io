/* offline shell · v22（v21 基础上只升版本号，强制用户拉到新 index.html）
   本次重点：
   1) server `/api/io-query` 修复出库金额字段名（带尾随空格「出库金额 」）；同时把
      单价(元）和单价（美元）两个 Lookup 字段展开为数值字段返回。
   2) PWA 出入库明细：数量统一 fmtRaw 原值（26880 不再缩成 2.7 万），
      金额 ¥/$ 双显示，新增「单价$」独立列。 */
const C='gzt-v22';
