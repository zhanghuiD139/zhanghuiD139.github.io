/* offline shell · v6（清掉 v5 及之前所有缓存；本次修 render 登录分支崩溃 → 全量重注册）
   策略：页面(index.html)与 data.json 走「网络优先 / 直连网络」，保证每次部署都能拿到最新代码与数据；
        静态资源(图标/manifest)走「缓存优先」，省流量。离线时回退缓存。
   文件名从 sw.v5.js 改为 sw.v6.js：旧 SW 缓存随之失效，强制全量重注册，根治登录页空白。 */
const C='gzt-v6';
...
self.addEventListener('activate',e=>{
  // 删除所有旧版本缓存（含 gzt-v1~v5 任意名），保证不残留冻结的旧 index.html / data.json
